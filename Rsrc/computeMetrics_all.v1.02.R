...
dataFstats.i <- dataX[dataX$yearSim>0]
...
#dataFstats.i <- dataFstats.i[dataFstats.i$speciesID==1] #pine 
dataFstats.i <- dataFstats.i[dataFstats.i$speciesID==2] #spruce 
...
write.csv(outputStats,file = 'statistics-alldata-spruce.csv')  #uncoment this two lines to save the csv file
write.csv(MSEdecomp, file = 'MSEdecomp-alldata-spruce.csv' ) #change the names to be saved as needed
...
MSEdecomp <- read.csv('MSEdecomp-all.csv')
...
plot.p1_p2<-annotate_figure(p2, top = text_grob('MSE decomposition'))#,
                                                #size = 12)
print(plot.p1_p2)
ggsave('MSE_decomp.jpg',plot = plot.p1_p2) #uncomment this line to save the Figures
...
Statistics <- read.csv('statistics-all.csv')
...
plot.Final<-annotate_figure(p.Final, top = text_grob('Statistics', size = 12))
print(plot.Final)

ggsave('Statistics.jpg',plot = plot.Final) #uncomment this line to save the Figures
rm(g_N,g_V,g_R,plot_g_N.RMSE,plot.g_N.MSE,plot.g_R.RMSE,plot.g_R.MSE,plot.g_V.RMSE,plot.g_V.MSE,plot.Final,p.Final,p1.RMSE,p2.RMSE,p1.MSE,p2.MSE,plot.r2,plot.rRMSE,plot.nRMSE,plot.pBIAS)
